...
/* DOM variables */
var starred = document.getElementById('today_starred');
var subs = document.getElementById('today_subs');
var others = document.getElementById('today_others');
...
function toggleEventCard(ind)
{
	...
	{
		today_starred.removeChild(event_cards[ind]);
	}
	else if (is_event_subd[ind])
	{
		today_subs.removeChild(event_cards[ind]);
    }
    else
    {
        today_others.removeChild(event_cards[ind]);
    }
	is_event_strd[ind] = !is_event_strd[ind];
	event_cards[ind] = createEventCard(ind);
	if(is_event_strd[ind])
	{
		today_starred.appendChild(event_cards[ind]);
	}
	else if (is_event_subd[ind])
	{
		today_subs.appendChild(event_cards[ind]);
    }
    else
    {
        today_others.appendChild(event_cards[ind]);
    }
}
...
for(let i = 0; i < event_names.length; ++i)
{
	...
	if(is_event_strd[i])
	{
		event_cards.push(today_starred.appendChild(card));
	}
	else if (is_event_subd[i])
	{
		event_cards.push(today_subs.appendChild(card));
    }
    else
    {
        event_cards.push(today_others.appendChild(card));
    }
}